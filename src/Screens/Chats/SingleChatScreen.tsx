...
const SingleChatScreen: FC<SingleChatScreenProps> = ({ route, navigation }) => {
  ...
  const params = route && route.params && route.params.params ? route.params.params : route.params;
  ...
  const deleteCollection = (collectionPath: string, batchSize: any) => {
    setLoading(true);
    const collectionRef = firestore().collection(collectionPath).doc(params.chatId);
    const query = collectionRef.collection(`${chat.subcollection}${'_messages'}`).limit(batchSize);
    return new Promise((resolve, reject) => {
        ...
        <BackgroundLayout title={`${route?.params?.title} Chat`}>
          ...
          <Modal visible={chatModalVisible} style={{ width: '60%' }}>
            <Card disabled={true}>
              ...
              >
                <>
                  <Text style={[styles.btnActionsTxt, { color: '#404040' }]}>CANCEL</Text>
                </>
              </Button>
            </Card>
          </Modal>
