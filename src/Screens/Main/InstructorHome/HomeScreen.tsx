...
const HomeScreen = () => {
    ...
    const RightActions = (dragX: any, item) => {
        ...
        return (
            <View style={{ flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <TouchableOpacity
                    onPress={() => navigation.navigate('StudentLocationScreen', {
                        student: item
                    })}
                    style={{
                        padding: 10,
    ...
    return (
        <>
            <WelcomeMessageModal />
            {!!selectedDependent && (
                <EditDependentModal selectedDependent={selectedDependent} setSelectedDependent={setSelectedDependent} />
            )}
                ...
                {!thumbnail ? <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <Swipeable
                            ...
                            <TouchableOpacity
                                style={[styles.item, { backgroundColor: index % 3 === 0 ? 'lightgreen' : index % 2 === 0 ? '#F6DDCC' : '#fff' }]}
                                onPress={() => navigation.navigate('Activity', {
                                    dependent: item
                                })}
                            >
                                <Text style={[styles.text, { fontWeight: '600' }]}>{`${item.firstName} ${item.lastName}`}</Text>
