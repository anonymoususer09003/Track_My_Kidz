...
import { TEST_APPROVALS } from '@/Constants';
import { StackNavigationProp } from '@react-navigation/stack';
import { MainStackNavigatorParamsList } from '@/Navigators/Main/RightDrawerNavigator';

const _approvals = TEST_APPROVALS;

const InstructorGroupApprovalScreen = () => {
  const [{ selectedActivity }, _dispatch]: any = useStateValue();
  const navigation = useNavigation<StackNavigationProp<MainStackNavigatorParamsList>>();
  const dispatch = useDispatch();
  const [approvals, setApprovals] = useState<any>(selectedActivity?.studentsActivity);
  const [rollCall, setRollCall] = useState<boolean>(false);
