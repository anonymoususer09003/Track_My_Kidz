...
const NotificationsScreen = () => {
  ...
  const notificationsLoading = useSelector(
    (state: {notifications: NotificationsState}) =>
      state.notifications.notificationsState.loading,
  );
  ...
  const renderItemIcon = (props, index) => {
    if (notifications.length - (index + 1) < unReadCount.latest) {
      return (
        <Icon
          {...props}
          style={[props.style, {padding: 5, marginRight: 15}]}
          name="bell"
        />
      );
    } else {
  ...
  const renderDate = (props, item) => {
    ...
    return isToday(date) ? (
      <Text>{format(date, "'Today at' HH:mm")}</Text>
    ) : (
      <Text>{format(date, "dd MMM yyyy 'at' HH:mm")}</Text>
    );
  };
  ...
  const NotificationItem = ({item, index}) => (
    <ListItem
      ...
      style={{
        backgroundColor:
          notifications.length - (index + 1) < unReadCount.latest
            ? Colors.primaryTint
            : Colors.white,
      }}
      key={index}
