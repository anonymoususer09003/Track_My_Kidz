...
import { GetFavoriteBlogs } from '@/Services/Favorites'
...
export default () => {
    ...
    const fetch = async (isFirst = false) => {
        if (isFirst) {
            const newBlogs = await GetFavoriteBlogs(searchParam, 1, 10)
            setPosts([...newBlogs])
            setPage(1)
        }
        else {
            const newBlogs = await GetFavoriteBlogs(searchParam, page, 10)
