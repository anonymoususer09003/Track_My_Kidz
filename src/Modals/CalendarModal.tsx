...
const CalendarModal = () => {
  ...
  const appointments = useSelector(
    (state: {appointments: AppointmentsState}) => state.appointments.item,
  );
  ...
  const selectedDayAppointments = appointments.filter(
    it =>
      new Date(it.appointmentTime).toDateString() ==
  ...
  function cancelAppointment(item: Appointment) {
    ...
    CancelAppointment(item.serviceProvider.id, item.id).then(
      data => {
        Toast.show({
          type: 'success',
    ...
    <Card
      ...
      <View
        ...
        <View style={{flex: 8, margin: 5, flexDirection: 'column'}}>
          ...
          <Text category="p1">{item.serviceProvider.user.username}</Text>
          <Text category="p1">
  ...
  const ScheduledLiveStream = ({item}) => (
    <Card
      ...
      <View
        ...
        <View style={{flex: 8, margin: 5, flexDirection: 'column'}}>
          <Text category="p1">{item?.creator?.username}</Text>
    ...
    return (
      <>
        <View style={styles.bottomContainer}>
          {selectedDayAppointments.length == 0 && selectedTab == 0 && (
            <Text style={{marginTop: 5}}>
              You have no scheduled{' '}
              {selectedTab == 0 ? 'service' : 'livestream'} for date selected
            </Text>
          )}
          {livestreams &&
            ...
            livestreams.filter(
              _livestream =>
                moment(_livestream.scheduledDate).format('yyyy-MM-dd') ===
    ...
    <TabBar
      ...
      {tabNames.map((tabName, index) => {
        if (selectedTab == index) {
          ...
          return (
            <View key={index} style={styles.topNavBackground}>
              <Button
          ...
          renderDay={({date}, style) => {
            if (selectedDate.toDateString() == date.toDateString()) {
              return (
                <View
                  style={[
                    SchedulerStyles.dayContainer,
                    style.container,
                    {backgroundColor: Colors.green},
                  ]}>
                  <Text
                    style={[
                      SchedulerStyles.dayValue,
                      style.text,
                      {color: Colors.white},
                    ]}>{`${date.getDate()}`}</Text>
                </View>
            ...
            return (
              <View style={[SchedulerStyles.dayContainer, style.container]}>
                <Text
                  style={[
                    SchedulerStyles.dayValue,
                    style.text,
