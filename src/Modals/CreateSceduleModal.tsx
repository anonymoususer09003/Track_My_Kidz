...
import {
  ...
  RadioGroup, Text,Datepicker,Select,Divider
...
const [selectedIndex, setSelectedIndex] = useState<number>(0);
    const [initialValues, setInitialValues] = useState<any>({eventName:"",venueName:'',eventAddress:'',date:'',time:'',eventType:"competition",});
...

{
 ...
 setSelectedIndex(isPractice?2:0)
  setInitialValues({
    eventName,venueName,eventAddress:venueAddress,date:new Date(date),time,eventType:isPractice?"practice": "competition",
  })
}
},[selectedScehdule])
console.log('selected scheudle',selectedScehdule)
return (
    <Modal
...
try{
          ...
          let res= selectedScehdule?await EditScheduleById(selectedScehdule.scheduleId,data):await CreateSchedule(values.eventType=='competition'?false:true,{...data})
            ...
            {({
               ...
               <Input
                    ...
                    <Input
                      ...
                      label={(evaProps: any) => (
                        <Text style={styles.inputLabels}>Venue name</Text>
                      )}
                    /> 
                       {errors.venueName && (
                      <Text style={styles.errorText}>
                      ...
                      <Text style={styles.errorText}>
                        {errors.eventAddress}
                          ...
                          <Datepicker
                            ...
                            label={(evaProps: any) => (
                                <Text style={styles.inputLabels}>Event Date</Text>
                              )}
...
<CustomTextDropDown    value={values.time}  placeholder="Time" dropDownList={timeStamp} 
onSelect={(name:any)=>    {  
...
save
                      ...
                      <View style={{marginVertical:10}}/>
                      <LinearGradientButton  onPress={closeModal}>
close
                      </LinearGradientButton>
