...
import { Linking, Pressable, StyleSheet, TouchableOpacity, View, ScrollView } from "react-native";
...
const TwoFactorAuthenticationModal = () => {
    ...
    const [qrUrl, setQrUrl] = useState("https://chart.googleapis.com/chart?chs=150x150&cht=qr&chl=&choe=UTF-8");
    ...
    const confirmSave = (code: any) => {
        ...
        if (user.isTwoFAConfirmed && !changingTwoFA) {
            ...
            ConfirmTwoFA(code).then(data => {
                ...
                dispatch(
                    ChangeModalState.action({ twoFactorAuthenticationModalVisibility: false }),
                )
                setCode('')
            })
        }
    ...
    return (
        ...
        >
            <Card style={styles.modal} disabled={true}>
                <ScrollView>
                    <TouchableOpacity style={{ position: 'absolute', top: 10, right: 0, zIndex: 10 }} onPress={() => {
                            dispatch(
                                ChangeModalState.action({ twoFactorAuthenticationModalVisibility: false }),
                            )
                        }}>
                    ...
                    <View style={styles.body}>
                        <View style={{ width: '85%', alignItems: 'center' }}>
                            ...
                            <>
                                ...
                                >You need to download Google or Microsoft Authenticator before enabling 2 Factor Authentication</Text>
                                <FastImage source={{
                                    uri: qrUrl
                                }} accessibilityLabel="Image" resizeMode={'contain'} style={styles.img} />
                                ...
                                <Text style={styles.label}
                                >Or Click Below To Copy Secret</Text>
                    ...
                    <View style={styles.bottom}>
                        <View style={styles.buttonText}>
                            ...
                            >
                                {user.isTwoFAConfirmed && !changingTwoFA ? 'Change Device' : 'Submit'}
                            </LinearGradientButton>
                        </View>
                    </View>
                </ScrollView>
...
const styles = StyleSheet.create({
    ...
    modal: {
        height: '90%', flexDirection: 'row', justifyContent: "center", alignContent: 'center', width: '100%',
        zIndex: 1
    },
    header: { flex: 1, flexDirection: 'row', textAlign: 'center', fontWeight: 'bold', fontSize: 20 },
    body: { flex: 4, flexDirection: 'row', justifyContent: "center", alignItems: 'center', },
    img: {
        minHeight: 220,
        width: 220,
        height: 220,
