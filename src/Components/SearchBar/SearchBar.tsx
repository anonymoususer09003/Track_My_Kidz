...
const SearchBar = ({ showDropdown, setShowDropdown, showFilter, searchText, onChangeText, onToggleChange, viewName, thumbnailView, isThumbnail, isThumbnailOnly }: SearchBarProps) => {
    const { Layout } = useTheme()
    const [thumbnail, setThumbnail] = useState(thumbnailView)
    ...
    const handleClearSearch = () => (
        searchText && searchText.length > 0 ?
            <TouchableOpacity onPress={() => onChangeText('')} style={{ flexDirection: 'column' }}>
        ...
        <View style={[Layout.alignItemsStart, { width: '100%', minHeight: 80 }]}>
            ...
            >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    {!isThumbnailOnly && <Input
                        value={searchText}
                        style={{ width: showFilter ? '65%' : showToggle ? '80%' : '100%', marginLeft: showFilter ? 20 : 0 }}
                        placeholder='Search'
